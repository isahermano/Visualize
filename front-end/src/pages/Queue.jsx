...
import { queueFunctions } from '../assets/assets';
     ...
     const { highlights } = useContext(AlgoContext);
      ...
      return (
          <div className='space-y-6'>
              <div className='min-h-[600px] border rounded-lg p-4 bg-white shadow'>
                  {renderQueueVisualization()}
              </div>
  
              {/* algorithm selection */}
              <div className='grid grid-cols-2 gap-3'>
                  {queueFunctions.map((algo) => (
                      <div key={algo.name} className='relative'>
                          <button
                              className={`border w-full rounded p-3 shadow transition-colors cursor-pointer font-bold ${selectedAlgo?.name === algo.name
              ...
              {/* display input only if algo selected */}
              {selectedAlgo && (
                  <div className=''>
                      {renderInputFields()}
                  </div>
